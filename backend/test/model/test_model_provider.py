...
class TestModelProvider(TestCase):
    home_predictor = Home()
    home_model = Model(name="home model", predictor=home_predictor)
    away_predictor = Away()
    away_model = Model(name="away model", predictor=away_predictor)
    ...
    def test_get_in_progress_predictor(self) -> None:
        self.assertEqual(
            self.provider.get_in_progress_predictor("away model"), self.away_predictor
        )
