...
class HistGradientBoostPredictor(Predictor):
    def __init__(
        self,
...
def build_model(
    results: List[Result],
) -> Tuple[HistGradientBoostingClassifier, OneHotEncoder]:
...
def train_HistGradientboost_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)
    return HistGradientBoostPredictor(model, team_encoding)
