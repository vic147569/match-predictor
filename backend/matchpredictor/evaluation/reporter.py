...
class Reporter:
    def __init__(
        self,
        title: str,
        validation_data: Iterable[Result],
    ...
    def __calculate_accuracy(self, model: Model) -> PredictionReport:
        accuracy, time_elapsed = Evaluator(model.predictor).measure_accuracy(
            self.validation_data
        )

        return PredictionReport(
            label=model.name, accuracy=accuracy, time_elapsed=time_elapsed
        )
