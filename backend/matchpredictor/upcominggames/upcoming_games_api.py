...
from matchpredictor.upcominggames.football_data_api_client import (
...
league_mapping = {
    LeagueMappingKey(
        areaName="Netherlands", competitionName="Eredivisie"
    ): "Dutch Eredivisie",
    LeagueMappingKey(
        areaName="England", competitionName="Championship"
    ): "English League Championship",
}


def response_from_football_data_matches(
    matches_response: FootballDataMatchesResponse,
) -> UpcomingGamesResponse:
    def build_upcoming_game(match_json: MatchJson) -> UpcomingGame:
        league_mapping_key = LeagueMappingKey(
            areaName=match_json.area.name, competitionName=match_json.competition.name
        )

        return UpcomingGame(
            league=league_mapping.get(
                league_mapping_key, league_mapping_key.default_value()
            ),
            home=match_json.homeTeam.shortName,
            away=match_json.awayTeam.shortName,
        )

    games = [build_upcoming_game(m) for m in matches_response.matches]
